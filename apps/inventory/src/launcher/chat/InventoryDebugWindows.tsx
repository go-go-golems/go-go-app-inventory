/*
 * Detached Event Viewer and Timeline Debug windows for the react-chat
 * Inventory chat path — the original os-chat look (family filter pills,
 * timestamped rows with per-family colored event types, expandable YAML
 * payloads, Pause / Clear / Hold / Follow Stream / Export YAML; two-pane
 * timeline with entity list + structured tree) rebuilt on chat-provider's
 * debug stream.
 *
...
const ALL_FAMILIES: InventoryDebugFamily[] = ['llm', 'tool', 'widget', 'timeline', 'ws', 'raw', 'other'];

const FAMILY_COLORS: Record<InventoryDebugFamily, string> = {
  llm: '#3b82f6',
  tool: '#f59e0b',
  widget: '#8b5cf6',
  timeline: '#10b981',
...
const FAMILY_LABELS: Record<InventoryDebugFamily, string> = {
  llm: 'LLM',
  tool: 'Tool',
  widget: 'WID',
  timeline: 'TL',
...
function isNearBottom(el: { scrollTop: number; clientHeight: number; scrollHeight: number }): boolean {
...
export function InventoryEventViewerWindow({ convId }: { convId: string }) {
  ...
  const visible = useMemo(() => {
    const filtered = filterVisibleEntries(entries, filters, hideTextPatch);
    return filtered.length > MAX_ENTRIES ? filtered.slice(filtered.length - MAX_ENTRIES) : filtered;
  }, [entries, filters, hideTextPatch]);
  ...
  const clearLog = useCallback(() => {
    ...
    setExpandedIds(new Set());
    setCopyFeedbackById({});
    inventoryChatDebugStore.clear(convId);
  }, [convId]);
  ...
  const exportVisibleToYaml = useCallback(() => {
    try {
      ...
      downloadYaml(`events-${convId.slice(0, 8)}-${exportedAt.replace(/[:.]/g, '-')}.yaml`, yaml);
...
// ---------------------------------------------------------------------------
// Timeline Debug (two-pane: entity list + tree/yaml detail)
// ---------------------------------------------------------------------------
...
export function InventoryTimelineDebugWindow({ convId, apiBasePrefix }: { convId: string; apiBasePrefix: string }) {
  ...
  const snapshot = useMemo(
    () => buildTimelineDebugSnapshot(convId, folded.mirror as never),
    [convId, folded.mirror],
  );
